...
def test_cost_unknown_renders_na_not_zero(tmp_path):
    """A client that reports no cost estimate (StubLLM has no last_call)
    makes the run total UNKNOWN — the brief must say n/a, not $0.00."""
    cfg = _write_config(tmp_path)
    result = run_batch(cfg, run_id="nocost", llm_client=StubLLM())

    assert result.rca_groups_investigated == 1
    assert result.cost_known is False
    assert result.cost_summary["cost_known"] is False
...
def test_cost_known_with_reporting_client(tmp_path):
    cfg = _write_config(tmp_path)
