...
1. Materializes every input source ONCE (eager `.collect().lazy()`, so file-backed sources aren't re-parsed per check-collect). A source that fails to read becomes a CRITICAL `source_unavailable` finding — an unreadable feed must never produce a silent "all clear".
...
4. Scores every Issue (by score MAGNITUDE — detectors store signed z-scores, thresholds are one-sided) and dedupes on `(entity_id, field_name, snapshot_date)` — the dedup key is the uuid5 `finding_id` computed by `compute_finding_id(...)`. Detectors WITHOUT a row in `DEFAULT_THRESHOLDS` are self-scoring: their own severity/confidence is preserved (`null_check` ERROR stays ERROR). The representative Issue is the highest-severity / highest-confidence member of the group; `Finding.detector_sources` lists every detector that flagged the point; a merge across tables records every table in `details["sources"]`. **Agreement boost:** when ≥2 distinct detector families flag the same point AND looked at the same data (same `__source__`, or a cross-source detector, which matches any source), confidence moves halfway toward 1.0 and `details["agreement_boost"]` records the families + sources + pre-boost value. Two families flagging a same-named field in different tables do NOT corroborate each other.
...
All runtime LLM calls go through Claude Code headless mode — `ClaudeCodeClient` shells out to `claude -p --output-format json` as a subprocess, passing the prompt on stdin (never argv: argv has a ~128 KiB per-argument ceiling and is world-readable via `/proc/<pid>/cmdline`). The JSON envelope carries both the model's text (`result`) and Claude Code's own per-call cost estimate (`total_cost_usd` + `usage`), so each call records its cost as a side effect (`CallCost` on the client) without LENS maintaining a price table. NO `anthropic` SDK is used; LENS environments authenticate via Claude Code SSO only. Both `src/lens/wiki/ingest.py` and `src/lens/rca/agent.py` accept any object conforming to the `LLMClient` Protocol, so unit tests inject deterministic stubs and never hit the network. `total_cost_usd` is a client-side ESTIMATE (not authoritative billing); the batch rolls fresh-investigation cost into `BatchResult.total_cost_usd` and the brief / digest / CLI surface it labeled "estimated". When any fresh investigation reports no estimate at all, `BatchResult.cost_known` flips false and the readouts show "n/a" rather than a misleading $0.00.
